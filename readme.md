# Mark-XLVI — Local Command Approval

This repository includes a safe, permissioned flow for running local commands requested by Jarvis.

Quick usage:

1. Queue a command (recommended):

```bash
python3 -c "from actions.computer_control import computer_control; print(computer_control({'command':'echo Hello from Jarvis'}))"
```

2. Approve the queued command:

```bash
python3 tools/approve_command.py --id <pending-id> --approve
```

3. To enable auto-approve for trusted callers, edit `config/approval.json` and add entries to `allowlist` or set a `passphrase`. Do NOT commit secrets.

Security:
- By default commands are queued to prevent accidental or remote code execution.
- Store sensitive API keys in `.env` and ensure `.env` is in `.gitignore`.
...
## 🎤 Voiceover (Gemini) Notes

- `actions/voiceover.py` saves generated audio to `voiceovers/voiceover_<uuid>.wav` and will try to play it using `afplay` on macOS if no UI `player` is provided.
- To test voiceover after setting `GEMINI_API_KEY` in your environment:

```bash
export GEMINI_API_KEY="YOUR_REAL_KEY"
python3 -c "import sys; sys.path.insert(0,'.'); from actions.voiceover import voiceover; print(voiceover({'text':'Hello from Jarvis','voice':'Charon'}))"
```

If you see an API key error from the command, rotate or set the correct key. The code reports clear errors for missing SDK or invalid keys.
